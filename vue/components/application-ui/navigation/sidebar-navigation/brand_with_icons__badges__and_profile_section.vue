    ...
    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
      ...
      <nav class="mt-5 flex-1 px-2 space-y-1" aria-label="Sidebar">
        <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-600 hover:bg-opacity-75', 'group flex items-center px-2 py-2 text-sm font-medium rounded-md']">
          <component :is="item.icon" class="mr-3 h-6 w-6 text-indigo-300" aria-hidden="true" />
          {{ item.name }}
          <span v-if="item.count" :class="[item.current ? 'bg-indigo-600' : 'bg-indigo-800', 'ml-auto inline-block py-0.5 px-3 text-xs font-medium rounded-full']">
            {{ item.count }}
          </span>
        </a>
      </nav>
