...
-->
<template>
  <Disclosure as="nav" class="bg-light-blue-500" aria-label="Global" v-slot="{ open }">
      ...
      <div class="flex justify-between h-16">
        <div class="flex items-center px-2 lg:px-0">
          ...
          <div class="hidden lg:ml-8 lg:flex lg:space-x-4">
            <a v-for="item in navigation" :key="item.name" :href="item.href" class="rounded-md py-2 px-3 text-sm font-medium text-white hover:bg-light-blue-400">
              {{ item.name }}
            </a>
          </div>
        </div>
        ...
        <div class="flex items-center lg:hidden">
          <!-- Mobile menu button -->
          <DisclosureButton class="inline-flex items-center justify-center p-2 rounded-md text-light-blue-200 hover:text-white hover:bg-light-blue-400 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
        ...
        <div class="hidden lg:ml-4 lg:flex lg:items-center">
          ...
          <Menu as="div" class="ml-4 relative flex-shrink-0">
            <div>
              <MenuButton class="bg-light-blue-500 rounded-full flex text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-light-blue-500 focus:ring-white">
    ...
    <DisclosurePanel class="lg:hidden">
      <div class="pt-2 pb-3 px-2 space-y-1">
        <a v-for="item in navigation" :key="item.name" :href="item.href" class="block rounded-md py-2 px-3 text-base font-medium text-white hover:text-white hover:bg-light-blue-400">{{ item.name }}</a>
      </div>
      <div class="pt-4 pb-3 border-t border-light-blue-500">
        ...
        <div class="mt-3 px-2">
          <a v-for="item in userNavigation" :key="item.name" :href="item.href" class="block rounded-md py-2 px-3 text-base font-medium text-light-blue-200 hover:text-white hover:bg-light-blue-400">{{ item.name }}</a>
        </div>
      </div>
    </DisclosurePanel>
  </Disclosure>
    ...
    <form>
      <div class="space-y-6">
        ...
        <div>
          ...
          <div class="mt-1">
            <textarea id="description" name="description" rows="3" class="block w-full shadow-sm focus:ring-light-blue-500 focus:border-light-blue-500 sm:text-sm border-gray-300 rounded-md" />
          </div>
        </div>
        ...
        <RadioGroup v-model="selected">
          ...
          <div class="mt-1 bg-white rounded-md shadow-sm -space-y-px">
            <RadioGroupOption as="template" v-for="(setting, settingIdx) in settings" :key="setting.name" :value="setting" v-slot="{ checked, active }">
              <div :class="[settingIdx === 0 ? 'rounded-tl-md rounded-tr-md' : '', settingIdx === settings.length - 1 ? 'rounded-bl-md rounded-br-md' : '', checked ? 'bg-light-blue-50 border-light-blue-200 z-10' : 'border-gray-200', 'relative border p-4 flex cursor-pointer focus:outline-none']">
                <span :class="[checked ? 'bg-light-blue-600 border-transparent' : 'bg-white border-gray-300', active ? 'ring-2 ring-offset-2 ring-light-blue-500' : '', 'h-4 w-4 mt-0.5 cursor-pointer rounded-full border flex items-center justify-center']" aria-hidden="true">
                  <span class="rounded-full bg-white w-1.5 h-1.5" />
                </span>
                <div class="ml-3 flex flex-col">
                  <RadioGroupLabel as="span" :class="[checked ? 'text-light-blue-900' : 'text-gray-900', 'block text-sm font-medium']">
                    {{ setting.name }}
                  </RadioGroupLabel>
                  <RadioGroupDescription as="span" :class="[checked ? 'text-light-blue-700' : 'text-gray-500', 'block text-sm']">
                    {{ setting.description }}
