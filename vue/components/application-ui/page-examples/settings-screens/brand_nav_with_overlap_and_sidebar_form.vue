...
<template>
  <div>
    <Disclosure as="div" class="relative bg-light-blue-700 pb-32 overflow-hidden" v-slot="{ open }">
      <nav :class="[open ? 'bg-light-blue-900' : 'bg-transparent', 'relative z-10 border-b border-teal-500 border-opacity-25 lg:bg-transparent lg:border-none']">
        <div class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
          <div class="relative h-16 flex items-center justify-between lg:border-b lg:border-light-blue-800">
            ...
            <div class="flex lg:hidden">
              <!-- Mobile menu button -->
              <DisclosureButton class="p-2 rounded-md inline-flex items-center justify-center text-light-blue-200 hover:text-white hover:bg-light-blue-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
            ...
            <div class="hidden lg:block lg:ml-4">
              <div class="flex items-center">
                ...
                <Menu as="div" class="relative flex-shrink-0 ml-4">
                  <div>
                    <MenuButton class="rounded-full flex text-sm text-white focus:outline-none focus:bg-light-blue-900 focus:ring-2 focus:ring-offset-2 focus:ring-offset-light-blue-900 focus:ring-white">
        ...
        <DisclosurePanel class="bg-light-blue-900 lg:hidden">
          <div class="pt-2 pb-3 px-2 space-y-1">
            <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-black bg-opacity-25' : 'hover:bg-light-blue-800', 'block rounded-md py-2 px-3 text-base font-medium text-white']">{{ item.name }}</a>
          </div>
          <div class="pt-4 pb-3 border-t border-light-blue-800">
            ...
            <div class="mt-3 px-2">
              <a v-for="item in userNavigation" :key="item.name" :href="item.href" class="block rounded-md py-2 px-3 text-base font-medium text-light-blue-200 hover:text-white hover:bg-light-blue-800">{{ item.name }}</a>
            </div>
          </div>
        </DisclosurePanel>
      </nav>
      <div :class="[open ? 'bottom-0' : 'inset-y-0', 'absolute flex justify-center inset-x-0 left-1/2 transform -translate-x-1/2 w-full overflow-hidden lg:inset-y-0']" aria-hidden="true">
